...
| A1d | The zoom LADDER + table-region anchors (the owner's exact walk is law) | table regions (deck · discard · dice · windows · global-play · standings · log · art-banner) are ANCHORS — click one, zoom in to read THAT section | anchor-READ ↔ anchor-SCENE ↔ OVERVIEW ↔ TABLE-READ on one wheel axis; read zoom-in DISABLED; the anchor survives the whole walk | clicks re-anchor (boards · felt · table regions) · wheel walks the ladder | 🔶 BUILT (I-66, VG8h reworked + VG8i, kill-first, 30/30) — awaiting the owner's playtest verdict |
...
| A9 | Hand fan (redaction-honest, I-59d) | fanned card meshes at the viewing seat only — OWNER-RULED 2026-08-02: FACE DOWN when not anchored, positioned to the RIGHT of the ledger | fan spread (spike-proven) · card raise on hover · face-down↔fan on anchor | click to zoom · anchor to reveal | ⬜ spike partial |
| A10 | Books panel (boty:books) | OWNER-RULED 2026-08-02: a physical LEDGER object sitting FLAT in front of the player that "flips open" to read (P&L, Balance sheet), then zooms via the ladder | flip-open · slide-in | tab = ui-state; fills from projection (I-56d); anchor + zoom to read | ⬜ |
