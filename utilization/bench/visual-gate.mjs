...
// measured BOOKS_PANEL def (I-56) with REAL balance-sheet fills from the projection.
// FOUR checks: (n1) the OPEN ledger renders BOOKS_PANEL's region ids (DOM-vs-LAW); (n2)
// the balance identity Assets ≡ Liabilities + Equity holds AND matches the INDEPENDENT
...
// VG8o — A15: THE GAME BOX (I-75, owner-ruled 2026-08-02; kill-first). An OPEN box (a
// base + four low walls) with a SEPARATE lid set OFF the base, to the RIGHT of the
// VIEWING seat's board, as if unpacked — STATIC (the unboxing animation is a recorded
// future upgrade). Selectable via the ladder (a click ANCHORS it), NOT fidgetable.
// Diffused (MeshBasic), unskinned (D-1). Asserted as scene STATE — object presence +
// world geometry + the anchor — never pixels (I-57c). All four kills run and recorded
// before this block's commit (drop the box → box-present; place it left of the board →
// box-right-of-board; lid ONTO the base → box-open-form; click doesn't anchor 'box' →
// box-selectable). Baseline restored → the gate greens with these four added.
{
  await page.evaluate(() => window.__GAME3D__.glideTo('overview'));
  await page.waitForFunction(() => !window.__GAME3D__.gliding(), null, { timeout: 60000 }).catch(() => {});
...
// ── VG8m — A4 (I-73; kill-first): THE ROUND-CARD SEQUENCE + a SEEDED ROLLING EXHIBIT
// die. Reload game3d for a clean genesis (moe active = the VIEWING seat). The die is an
// EXHIBIT — the slice has NO engine die verb — so it touches NO game state: rowHash AND
// moveCount are invariant through every roll and fidget. Assert geometry/verdict STATE,
// never pixels (I-57c); WAIT ON diePhase STATE, never clocks (I-60f).
{
  await page.goto(`http://localhost:${GATE_PORT}/game3d.html`);
  await page.waitForFunction(() => window.__GAME3D__ && window.__GAME3D__.ready(), null, { timeout: 30000 });
  await page.evaluate(() => window.__GAME3D__.glideTo('table'));
  ...
  await page.screenshot({ path: '/tmp/vg-3d-a4.png' });
}
