...
| boty:books (arc step 1) | packs/boty/src/layouts.ts (extended) | I-56b/c/d · EXT-5 F5 · inventory §3.4 · sources 10 (+18 pending for the callout) | Thin child of PANEL_PARENT (v1's Books IS the generic panel): title override to the "The books · {seat}" band; the P&L cash-vs-paper `callout` add lands when source 18 is measured (ABSENT until then — I-56b deferral). Tab switch = ui-state; invariants engine-side; bench fills real-from-projection (balance) / bracketed (P&L) | built |
